...
namespace SCCoreSystems
{
    public class sc_prep_projectile : MonoBehaviour
    {
        ...
        public float projectileimpulse = 10; //1050m/s
        ...
        //public sccsshadowbulletpoolerscript sccsbulletpoolerscriptgetter;
        ...
        public new static sc_prep_projectile sc_prep_projectile_;
        ...
        public int playerpressingtrigger = 0;

        void Start()
        {
            //bullet_template.tag = "bulletshot";
            sc_prep_projectile_ = this;
            someaudiosource = this.gameObject.GetComponent<AudioSource>();
            //laserLine = GetComponent<LineRenderer>();

            fpsCam = GetComponentInParent<Camera>();

            //sccsshadowbulletpoolerscriptgetter = new sccsshadowbulletpoolerscript();// this.GetComponent<sccsshadowbulletpoolerscript>();

            //sccsbulletpoolerscriptgetter = new sccsshadowbulletpoolerscript();
        ...
        void Fire()
        {
            GameObject obj = sccsshadowbulletpoolerscriptgetter.GetPooledObject();

            if (obj == null) return;

            obj.transform.position = gunEnd.transform.position;
        ...
        public void Update()
        {
           ...
           /* if (target != null)
            ...
            if (playerpressingtrigger == 1)
            {
                var ray = new Ray(transform.position, transform.forward);// Camera.main.ScreenPointToRay(Input.mousePosition);

                RaycastHit hit;
                if (Physics.Raycast(ray, out hit, 10000, layerMask))
                {
                    target = hit.transform;
                    rigidbodytarget = hit.transform.GetComponent<Rigidbody>();
                        ...
                        if (rigidbodytarget != null && target != null)
                        {
                            ...
                            if (counterbarrelR >= fireRate)
                            {
                                if (bullettypeselected == 0)
                                {
                                    ...
                                    bullet_shot = sccsshadowbulletpoolerscriptgetter.GetPooledObject();
                                    bullet_template = unitytutorialgameobjectpool.GetPooledObject();
                                    bullet_template.GetComponent<shadowBullet>().shadowProjectile = bullet_shot;

                                    bullet_shot.transform.position = gunEnd.transform.position;
                                    bullet_shot.transform.rotation = gunEnd.transform.rotation;
                                    bullet_template.transform.position = gunEnd.transform.position;
                                    bullet_template.transform.rotation = gunEnd.transform.rotation;


                                    bullet_template.GetComponent<shadowBullet>().projectileimpulse = projectileimpulse;
                                    bullet_template.GetComponent<shadowBullet>().bullseyedirection = gunEnd.transform.forward;
                                    bullet_template.GetComponent<shadowBullet>().firing_ship = player.gameObject;
                                    bullet_template.GetComponent<shadowBullet>().gunEnd = gunEnd.gameObject;
                                    bullet_template.GetComponent<shadowBullet>().enabled = true;

                                    someaudiosource.PlayOneShot(someaudiosource.clip);


                                    bullet_shot.transform.parent = null;
                                    bullet_shot.tag = "bulletshot";
                                    bullet_shot.SetActive(true);
                                    bullet_template.SetActive(true);


                                    var shadowRigid = bullet_shot.GetComponent<Rigidbody>();
                                    shadowRigid.isKinematic = false;
                                    shadowRigid.AddForce(bullet_shot.transform.forward * projectileimpulse, ForceMode.Impulse);
                                    ...
                                    //bullet_shot.SetActive(true);
                                    // bullet_shot.transform.rotation = Quaternion.identity;

                                    //bullet_shot.GetComponent<shadowBullet>().shadowObject = sccsshadowbulletpoolerscriptgetter.GetPooledObject();

                                    /*bullet_template.GetComponent<shadowBullet>().shadowProjectile.transform.position = gunEnd.transform.position;
                                    bullet_shot.transform.position = gunEnd.transform.position;
                                    bullet_shot.transform.rotation = gunEnd.transform.rotation;

                                    bullet_template.transform.position = gunEnd.transform.position;
                                    bullet_template.transform.rotation = gunEnd.transform.rotation;
                                    //bullet_shot.GetComponent<shadowBullet>().shadowProjectile.transform.position = gunEnd.transform.position;
                                    bullet_shot.transform.parent = null;
                                    bullet_shot.tag = "bulletshot";
                                    ...
                                    /*bullet_template.GetComponent<shadowBullet>().bullseyedirection = bullet_shot.transform.forward;
                                    bullet_template.GetComponent<shadowBullet>().firing_ship = player.gameObject;
                                    bullet_template.GetComponent<shadowBullet>().gunEnd = gunEnd.gameObject;
                                    bullet_template.GetComponent<shadowBullet>().enabled = true;
                                ...
                                */
                                    ...
                                    */
                                    ...
                                    //bullet_shot = Instantiate(bullet_template, bullet_template.transform.position, bullet_template.transform.rotation);
                                    bullet_shot= sccsshadowbulletpoolerscriptgetter.GetPooledObject();
                                    bullet_shot.GetComponent<shadowBullet>().shadowProjectile = bullet_shot;
                                    //bullet_shot.SetActive(true);
                            ...
                            if (counterbarrelR >= fireRate)
                            {
                                if (bullettypeselected == 0)
                                {
                                   ...
                                   // bullet_shot.transform.rotation = Quaternion.identity;
                                    ...
                                    //bullet_shot.GetComponent<shadowBullet>().projectileimpulse = projectileimpulse;
                                    bullet_shot.GetComponent<shadowBullet>().bullseyedirection = bullet_shot.transform.forward;
                                    bullet_shot.GetComponent<shadowBullet>().firing_ship = player.gameObject;
                                    bullet_shot.GetComponent<shadowBullet>().gunEnd = gunEnd.gameObject;
                                    bullet_shot.GetComponent<shadowBullet>().enabled = true;
