            ...
            if (byteCheck != null)
            {
                ...
                if (!resultsOne && !resultsThree && !resultsTwo)
                {
                    //if (byteCheck.worldChunk[byteCheck.smallX + realChunkWidth * (byteCheck.smallY + realChunkHeight * byteCheck.smallZ)] == null)
                    {
                        ...
                        GameObject currentChunk = (GameObject)Instantiate(chunktospawn, new Vector3(meshDator.trueRealPos.x, meshDator.trueRealPos.y, meshDator.trueRealPos.z), Quaternion.identity);
