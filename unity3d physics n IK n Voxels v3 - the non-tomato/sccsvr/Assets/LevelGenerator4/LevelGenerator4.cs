...
public class LevelGenerator4 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (counter == 2)
        {
            ...
            if (countingCoroutinesStart == countingCoroutinesEnd)
            {
                ...
                var listOfObjs = GameObject.FindGameObjectsWithTag("chunks");

                for (int i = 0; i < listOfObjs.Length; i++)
                {

                    listOfObjs[0].transform.parent = this.gameObject.transform;
                }
    ...
    IEnumerator buildFaces()
    {
        for (int i = 0; i < chunkz.Length; i++)
        {
            ...
            chunkPos = singleChunk.transform.position;

            if (singleChunk.GetComponent<newFloorTiles>() != null)
            {
                singleChunk.GetComponent<newFloorTiles>().Regenerate();
            }

            //singleChunk.GetComponent<newFloorTiles>().Regenerate();// _buildFaces(chunkPos);
