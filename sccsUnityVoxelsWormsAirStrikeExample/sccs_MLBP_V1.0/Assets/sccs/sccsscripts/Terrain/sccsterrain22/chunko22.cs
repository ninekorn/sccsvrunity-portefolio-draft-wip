...
public class chunko22 : MonoBehaviour
{
    public int width = 20;  // 32
    public int height = 20; // 32
    public int depth = 20;  // 32
    public static float planeSize = 0.1f; // 0.125f                                 
   ...
   float stoneMinHeight = -12;
    ...
    void Start()
    {
        ...
        Regenerate();
        GetComponent<chunko22>().enabled = false;
    }
    ...
    public void DrawBrick(int x, int y, int z)
    {
        ...
        /*//LEFTFACE
		if (x == 0 && test.getChunk(Mathf.RoundToInt(transform.position.x - 1), Mathf.RoundToInt(transform.position.y), Mathf.RoundToInt(transform.position.z)) != null)
        {
            //Instantiate(sphere,new Vector3(x,y,z)*planeSize+transform.position,Quaternion.identity);
			mainChunk chuk = test.getChunk(Mathf.RoundToInt(transform.position.x - 1), Mathf.RoundToInt(transform.position.y), Mathf.RoundToInt(transform.position.z));
		...
		//FRONTFACE
		if (y == height-1 && test.getChunk(transform.position.x, transform.position.y+1, transform.position.z) != null)
		{
		...
		if (y == 0 && test.getChunk(transform.position.x, transform.position.y-1, transform.position.z) != null)
		{
			...
			if (chuk.chunker.GetComponent<chunku>().GetByte(x,height-1,z) == 0)
			{
				//float xx = (Mathf.Floor(start.x * fraction) / fraction) + chunkRadius;
				//float yy = (Mathf.Floor(start.y * fraction) / fraction) + chunkRadius;
				//float zz = (Mathf.Floor(start.z * fraction) / fraction) + chunkRadius;
