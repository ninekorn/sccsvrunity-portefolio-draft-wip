...
public class sccsInstancesunitypool : MonoBehaviour
{
   

    public static sccsInstancesunitypool current;
    ...
    private void Start()
    {
        ...
        objMesh = pooledObject.transform.gameObject.GetComponent<MeshFilter>().sharedMesh;
                ...
                for (int yy = 0; yy < height; yy++)
                {
                    for (int zz = 0; zz < depth; zz++)
                    {
                        ...
                        if (batchIndexNum < width * height * depth)
                        {
                            ...
                            someInstances[i][indexOfinstancedObject].enabled = -1;
                            someInstances[i][indexOfinstancedObject].pos = _chunkPos;
                            someInstances[i][indexOfinstancedObject].rot = Quaternion.identity;
                            someInstances[i][indexOfinstancedObject].scale = new Vector3(1, 1, 1);
                            someInstances[i][indexOfinstancedObject].gameobjectoinstanceindex = i;
                            someInstances[i][indexOfinstancedObject].instanceindex = indexOfinstancedObject;
                            someInstances[i][indexOfinstancedObject].currentInstanceGameObject = null;
                            someInstances[i][indexOfinstancedObject].enabled = -1;
                            someInstances[i][indexOfinstancedObject].swap = -1;
                            someInstances[i][indexOfinstancedObject].initGametime = -1;
                            someInstances[i][indexOfinstancedObject].InitInstancePositionToLinkTo = _chunkPos;
                            someInstances[i][indexOfinstancedObject].instanceenabledcounter = 0;
                            someInstances[i][indexOfinstancedObject].instanceenabledcounterMax = 1;
                            someInstances[i][indexOfinstancedObject].instanceenabledcounterSwtc = 0;
    ...
    public void addObjectInstanceToDrawingList(instancedata instancedata_)
    {
        ...
        //Debug.Log("length:" + someInstances.Length + "/batchobjectindex:" + batchobjectindexdatain);
        if (batchobjectindexdatain < someInstances.Length)
        {
            Debug.Log("0new instance request");
            if (someInstances[batchobjectindexdatain] != null)
            {
                Debug.Log("1new instance request");
                if (someInstances[batchobjectindexdatain][batchinstanceindexdatain].enabled == -1)
                {
                    Debug.Log("2new instance request");
                    ...
                    someInstances[batchobjectindexdatain][batchinstanceindexdatain].enabled = 0;
                    sccsInstancesunitypool0.current.addObjectInstanceToDrawingList(instancedata0);
                    batchinstanceindexdatain++;
                }
            }
        }
        else
