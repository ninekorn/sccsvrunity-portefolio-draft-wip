...
public class sccsproceduralplanetbuilder : MonoBehaviour
{
    ...
    //byte block;
    public static int someotherrealplanetwidth = 4;
    ...
    /*public sccsproceduralplanetbuilderGen2 currentplanetbuilder
    {
        get
        {
            if (sccsproceduralplanetbuilderGen2staticscriptlock == null)
            {
                sccsproceduralplanetbuilderGen2staticscriptlock = new sccsproceduralplanetbuilderGen2();
                return sccsproceduralplanetbuilderGen2staticscriptlock;
            }
            else
            {
                return sccsproceduralplanetbuilderGen2staticscriptlock ;
    ...
    public static sccsproceduralplanetbuilder sccsproceduralplanetbuilderGen2staticscriptlock;

    //public sccsproceduralplanetbuilderGen2 
    Vector3 parentPosition;

    void Awake()
    {

        sccsproceduralplanetbuilderGen2staticscriptlock = this;
    ...
    public void CreateEmptyObjects()
    {
        ...
        //Vector3 planetchunkpos = new Vector3((transform.position.x) + (posX * realplanetwidth), (transform.position.y) + (posY * realplanetwidth), (transform.position.z) + (posZ * realplanetwidth));
        Vector3 planetchunkpos = new Vector3((transform.position.x) + (posX * realplanetwidth * planeSize), (transform.position.y) + (posY * realplanetwidth * planeSize), (transform.position.z) + (posZ * realplanetwidth * planeSize));
    ...
    private void Update()
    {
        ...
        if (initcounterSwtc == 1)
        {
            ...
            _main_received_messages[0].planetChunkPos = transform.position;
            someobjArray[0] = _main_received_messages;


            //Vector3 planetchunkpos = transform.position;// new Vector3((transform.position.x) + (posX * realplanetwidth * planeSize), (transform.position.y) + (posY * realplanetwidth * planeSize), (transform.position.z) + (posZ * realplanetwidth * planeSize));
                ...
                sccssometask = Task<object[]>.Factory.StartNew((tester0001) =>
                {
                    ...
                    while (taskcancelFlagTwo == 0)
                    {
                        if (counterCreateChunkObjectByteMap < total)
                        {
                            ...
                            int _index = xx + (ChunkWidth) * (yy + (ChunkHeight) * zz);
                            Vector3 planetchunkmaincenterpos = _main_received_messages[0].planetChunkPos;
                            float posX = (xx);
                            float posY = (yy);
                            float posZ = (zz);
                            ...
                            float posX = 0;// (tempx * planeSize) + (worldposition.x) + ((width / 2) * planeSize); //((width / 2) * planeSize)
                            float posY = 0;//(tempy * planeSize) + (worldposition.y) + ((width / 2) * planeSize);
                            float posZ = 0;//(tempz * planeSize) + (worldposition.z) + ((width / 2) * planeSize);


                            if (planetchunkmaincenterpos.x < 0)
                            {
                                worldposition.x += 4;
                                posX = (tempx * planeSize) + (worldposition.x) + ((width / 2) * planeSize);
                            }
                            else
                            {
                                posX = (tempx * planeSize) + (worldposition.x);
                            }

                            if (planetchunkmaincenterpos.y < 0)
                            {
                                worldposition.y += 4;
                                posY = (tempy * planeSize) + (worldposition.y) + ((width / 2) * planeSize);
                            }
                            else
                            {
                                posY = (tempy * planeSize) + (worldposition.y);
                            }

                            if (worldposition.z < 0)
                            {
                                worldposition.z += 4;
                                posZ = (tempz * planeSize) + (worldposition.z) + ((width / 2) * planeSize);
                            }
                            else
                            {
                                posZ = (tempz * planeSize) + (worldposition.z);
                            }*/


                            Vector3 positionindexed = new Vector3(posX * 4, posY * 4, posZ * 4);

                            //Vector3 realposition = new Vector3(posX * sccslodchunk.planeSize, posY * sccslodchunk.planeSize, posZ * sccslodchunk.planeSize);


                            //planetchunkpos = new Vector3((transform.position.x) + (posX * realplanetwidth * planeSize), (transform.position.y) + (posY * realplanetwidth * planeSize), (transform.position.z) + (posZ * realplanetwidth * planeSize));
                            _main_received_messages[0].planetChunkPos.x = _main_received_messages[0].planetChunkPos.x + (posX * 4 * planeSize);
                            _main_received_messages[0].planetChunkPos.y = _main_received_messages[0].planetChunkPos.y + (posY * 4 * planeSize);
                            _main_received_messages[0].planetChunkPos.z = _main_received_messages[0].planetChunkPos.z + (posZ * 4 * planeSize);
                            ...
                            if (_index < total)
                            {
                                ...
                                somemsg[0].chunkByteMaps[_index] = buildchunkmap(positionindexed, planetchunkmaincenterpos, _main_received_messages[0].planetChunkPos, Vector3.zero);
                            ...
                            Vector3 planetchunkmaincenterpos = _main_received_messages[0].planetChunkPos;
                            //planetchunkpos = new Vector3((transform.position.x) + (posX * realplanetwidth * planeSize), (transform.position.y) + (posY * realplanetwidth * planeSize), (transform.position.z) + (posZ * realplanetwidth * planeSize));
                            _main_received_messages[0].planetChunkPos.x = _main_received_messages[0].planetChunkPos.x + (posX * realplanetwidth * planeSize);
                            _main_received_messages[0].planetChunkPos.y = _main_received_messages[0].planetChunkPos.y + (posY * realplanetwidth * planeSize);
                            _main_received_messages[0].planetChunkPos.z = _main_received_messages[0].planetChunkPos.z + (posZ * realplanetwidth * planeSize);
                ...
                if (startOnceSwtc == 3)
                {
                    ...
                    for (int i = 0; i < total; i++)
                    {
                        if (blockers[i] != null)
                        {
                            ...
                            {
                                if (blockers[i].somesccsplanetchunk != null)
                                {
                                    blockers[i].somesccsplanetchunk.map = _main_received_messages[0].chunkByteMaps[i];
                                }
                            }
                        }
                    }
        ...
        if (_index < total)
        {
            if (pooledObjects.Count > 0)
            {
                ...
                if (somePooledObject != null)
                {
                    ...
                    var somescriptsccslodchunk = somePooledObject.GetComponent<sccsplanetchunk>();

                    somescriptsccslodchunk.sccsCustomStart(this);

                    Vector3 realIndexedPosition = new Vector3(posX, posY, posZ);
                    //somescriptsccslodchunk.realIndexedPosition = realIndexedPosition;

                    blockers[_index] = new mainChunk(planetchunkpos, somePooledObject, somescriptsccslodchunk, _index);
    ...
    void CreateChunkFaces()
    {
        ...
        if (_index < total)
        {
            if (blockers[_index] != null)
            {
                ...
                {
                    if (blockers[_index].somesccsplanetchunk != null)
                    {
                        blockers[_index].somesccsplanetchunk.sccsSetMap();
                        blockers[_index].somesccsplanetchunk.Regenerate();

                        if (blockers[_index].somesccsplanetchunk.vertexlist.Count > 0)
                        {
                            blockers[_index].somesccsplanetchunk.buildMesh();
        ...
        for (int t = 0; t < total; t++)
        {
            ...
            if (_index < total)
            {
                ...
                if (unityTutorialPooledGameObject.GetComponent<sccslodchunk>() != null)
                {
                    ...
                    blockers[_index] = new mainChunk(planetchunkpos, unityTutorialPooledGameObject.gameObject, unityTutorialPooledGameObject.GetComponent<sccslodchunk>());

                    //TO READD
                    //blockers[_index].somesccsplanetchunk.buildchunkmap(planetchunkpos);
                    //TO READD
    ...
    /*
public int smallchunkwidth = 16;
public int smallchunkheight = 16;
public int smallchunkdepth = 16;

public int[] buildchunkmapnewfail(Vector3 positionindexed, Vector3 worldposition, Vector3 planetcenterpos)
{
    ...
    int _index = xxx + (smallchunkwidth) * (yyy + (smallchunkheight) * zzz);
    ...
    zzz++;
    if (zzz >= (smallchunkdepth))
    {
        yyy++;
        zzz = 0;
        switchYYY = 1;
    }
    if (yyy >= (smallchunkheight) && switchYYY == 1)
    {
        xxx++;
        yyy = 0;
        switchYYY = 0;
    ...
    if (xxx >= (smallchunkwidth) && switchXXX == 1)
    {
        xxx = 0;
        switchXXX = 0;
        //break;
    ...
    public int[] buildchunkmap(Vector3 positionindexed, Vector3 planetchunkmaincenterpos, Vector3 worldposition, Vector3 planetcenterpos) //sccsproceduralplanetbuilder componentParent_, 
    {
        ...
        radiusplanetmountainend = ChunkWidth - 1;// + offsetDist;
        //worldposition.x += 1 / 2;
        //worldposition.y += 1 / 2;
        //worldposition.z += 1 / 2;
        ...
        {
            //float noiseX = Mathf.Abs(((float)(x * planeSize + worldposition.x + seed) / detailScale) * heightScale);

            for (int y = 0; y < height; y++)
            {
                //float noiseY = Mathf.Abs(((float)(y * planeSize + worldposition.y + seed) / detailScale) * heightScale);

                for (int z = 0; z < depth; z++)
                {
                    //float noiseZ = Mathf.Abs(((float)(z * planeSize + worldposition.z + seed) / detailScale) * heightScale);
                    ...
                    posX = (x * planeSize) + (worldposition.x);
                    posY = (y * planeSize) + (worldposition.y);
                    posZ = (z * planeSize) + (worldposition.z);
                    ...
                    /*Vector3 test = planetchunkmaincenterpos;

                    if (test.x < 0)
                    {
                        planetchunkmaincenterpos.x -= ChunkWidth;
                    }
                    if (test.y < 0)
                    {
                        planetchunkmaincenterpos.y -= ChunkWidth;
                    }
                    if (test.z < 0)
                    {
                        planetchunkmaincenterpos.z -= ChunkWidth;
                    }*/



                    //float posX = (x * planeSize) + (worldposition.x);// (tempx * planeSize) + (worldposition.x) + ((width / 2) * planeSize); //((width / 2) * planeSize)
                    //float posY = (y * planeSize) + (worldposition.y);//(tempy * planeSize) + (worldposition.y) + ((width / 2) * planeSize);
                    //float posZ = (z * planeSize) + (worldposition.z);//(tempz * planeSize) + (worldposition.z) + ((width / 2) * planeSize);



                    Vector3 pos = new Vector3(posX, posY, posZ);
                    //float distance = Vector3.Distance(pos, planetchunkmaincenterpos);
                    float distance = SCCoreSystems.sc_maths.sc_check_distance_node_3d(planetcenterpos, pos, 12, 12, 12, 12, 12, 12, 12, 12, 12);//Vector3.Distance(pos, planetcenterpos);
                    ...
                    temporaryY *= (Mathf.PerlinNoise((x * planeSize + worldposition.x + seed) / detailScale1, (z * planeSize + worldposition.z + seed) / detailScale1) * heightScale1);
                    float size0 = (1 / planeSize) * worldposition.y;
                    temporaryY -= size0;

                    temporaryX *= (Mathf.PerlinNoise((y * planeSize + worldposition.y + seed) / detailScale1, (z * planeSize + worldposition.z + seed) / detailScale1) * heightScale1);
                    float size1 = (1 / planeSize) * worldposition.x;
                    temporaryX -= size1;

                    temporaryZ *= (Mathf.PerlinNoise((x * planeSize + worldposition.x + seed) / detailScale1, (y * planeSize + worldposition.y + seed) / detailScale1) * heightScale1);
                    float size2 = (1 / planeSize) * worldposition.z;
                    temporaryZ -= size2;*/
                    ...
                    {
                        if (distance <= radiusplanetmountainend * 10) //20
                        {
                            //map[indexOf] = 1;
                        }
