...
namespace SCCoreSystems
{
    public static class sc_maths
    {
        ...
        //MODIFIED 2D TO 3D VERSION OF SEBASTIEN LAGUE WITH SOME MODS SIMPLY FOR VISUALLY BEING ABLE TO MODIFY TO ELLIPSOID AND OTHER GEOMETRY FORMS - it kinda works but ive got a hard time getting a perfect sphere. im not a mathematician
        //and i am a lazy programmer.
        public static float sccscheckdistancenode3dRadius(Vector3 nodeA, Vector3 nodeB, float distanceForMaxRadius) // i was thinking about using the index instead and then was like well i need the distance man.
        {
            //var pointFrontX = (1 * Math.cos(radToDeg * Math.PI / 180));
            //var pointFrontY = (1 * Math.sin(radToDeg * Math.PI / 180));
            ...
            if (dstX > dstZ)
            {
                dstX_vs_dstZ = distanceForMaxRadius * dstZ + distanceForMaxRadius * (dstX - dstZ);
            }
            else
            {
                dstX_vs_dstZ = distanceForMaxRadius * dstX + distanceForMaxRadius * (dstZ - dstX);
            }

            if (dstX > dstY)
            {
                dstX_vs_dstY = distanceForMaxRadius * dstY + distanceForMaxRadius * (dstX - dstY);
            }
            else
            {
                dstX_vs_dstY = distanceForMaxRadius * dstX + distanceForMaxRadius * (dstY - dstX);
            }

            if (dstY > dstZ)
            {
                dstY_vs_dstZ = distanceForMaxRadius * dstZ + distanceForMaxRadius * (dstY - dstZ);
            }
            else
            {
                dstY_vs_dstZ = distanceForMaxRadius * dstY + distanceForMaxRadius * (dstZ - dstY);
            }
