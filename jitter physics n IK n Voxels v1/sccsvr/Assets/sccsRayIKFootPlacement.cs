...
public class sccsRayIKFootPlacement : MonoBehaviour
{
    ...
    public int counterForIkFootPlacementMax = 1;
                    ...
                    if (raylengthTwo < raycounterLoopMaxTwo)
                    {
                        ...
                        if (chosenraycasthit != null)
                        {
                            ...
                            if (chosenraycasthit.Rigidbody != null && chosenraycasthit.Rigidbody.Shape.rIndex != -1 && chosenraycasthit.Rigidbody.Shape.rTag != 1)//Physics.Raycast(ray, out hit, 0.1f))
                            {
                                ...
                                float distanceNew = sc_maths.sc_check_distance_node_3d(lastFrameHitPoint, legstaticpivot.position, 10, 10, 10, 10, 10, 10, 10, 10, 10);
                                ...
                                /*Plane somePlane = new Plane(chosenraycasthit.Normal, 10);

                                //Ray someOtherRay = new Ray(legstaticpivot.position,legstaticpivot.forward);
                                Ray someOtherRay = new Ray(legstaticpivot.position + (legstaticpivot.forward), legstaticpivot.forward * 0.05f);

                                float someEnter = 0;
                                bool isIntersecting = somePlane.Raycast(someOtherRay, out someEnter);
                                ...
                                //
                                //Math3d.LineLineIntersection(out intersection)

                                //Vector3 testVec = Math3d.ProjectVectorOnPlane(chosenraycasthit.Normal, (legstaticpivot.forward));


                                ///Vector3 closestPointOnLine1;
                                //Vector3 closestPointOnLine2;
                                //Math3d.ClosestPointsOnTwoLines(out closestPointOnLine1,out closestPointOnLine2, legstaticpivot.position,,lastFrameHitPoint,);
                                Vector3 dir1 = (legstaticpivot.position + legstaticpivot.forward) - legstaticpivot.position;
                                dir1.Normalize();

                                Vector3 dir2 = (chosenraycasthit.Point- legstaticpivot.position);
                                float someLength = dir2.magnitude;
                                dir2.Normalize();


                                Vector3 crossProd = Vector3.Cross(chosenraycasthit.Normal, (-Vector3.forward));// Vector3.Cross(dir2, chosenraycasthit.Normal);
                                crossProd.Normalize();

                                Vector3 someDir = chosenraycasthit.Point - crossProd;
                                someDir.Normalize();

                                Debug.DrawRay(chosenraycasthit.Point, (crossProd), Color.red, 0.1f);
                                Vector3 crossProdTwo = Vector3.Cross(dir2, (someDir));
                                crossProdTwo.Normalize();
                                Debug.DrawRay(chosenraycasthit.Point, (crossProdTwo), Color.blue, 0.1f);
                                //float someAngle = Math3d.AngleVectorPlane(dir2, chosenraycasthit.Normal);
                                float someAngle =(float)sc_maths.AngleBetweener(dir2, crossProd, false);
                                //Vector3 vecNew = new Vector3(Mathf.Cos(someAngle), Mathf.Sin(someAngle), 0);


                                //Vector3 intersect;
                                //Math3d.LineLineIntersection(out intersect, chosenraycasthit.Point, crossProd, legstaticpivot.position, legstaticpivot.forward);

                                Vector3 intersect;

                                Math3d.LinePlaneIntersection(out intersect, chosenraycasthit.Point, crossProd, chosenraycasthit.Normal, chosenraycasthit.Point);



                                var rayThree = new Ray(legstaticpivot.position, legstaticpivot.forward);
                                var planer = new Plane(new Vector3(chosenraycasthit.Point.x, chosenraycasthit.Point.y, chosenraycasthit.Point.z), chosenraycasthit.Normal);

                                //bool someintersecter = rayThree.Intersects(ref planer, out intersect);
                                float someDist;
                                planer.Raycast(rayThree, out someDist);





                                float lastSide = Mathf.Cos(someAngle) * someLength;
                                Vector3 someDirTwo = (legstaticpivot.position + (legstaticpivot.forward * lastSide));
                                someDirTwo.Normalize();

                           
                                //Debug.DrawRay(legstaticpivot.position, (-someDirTwo), Color.white, 0.1f);

                                Vector3 testVec = Math3d.ProjectVectorOnPlane(chosenraycasthit.Normal, (legstaticpivot.forward));

                                //Debug.DrawRay(testVec, (legstaticpivot.forward), Color.white, 0.1f);
                                //Debug.DrawRay(legstaticpivot.position, (legstaticpivot.forward), Color.white, 0.1f);

                                //RAYTWO
                                var rayTwo = new Ray(legstaticpivot.position + (legstaticpivot.forward * (clampedDistance * 0.075f)), legstaticpivot.forward * 0.05f); //clampedDistanceTwo
                                //Debug.DrawRay(legstaticpivot.position + (legstaticpivot.forward * (clampedDistanceTwo * 0.075f)), legstaticpivot.forward * 0.05f, Color.magenta, 0.1f);

                                var someray = Raycast(rayTwo, raylengthTwo, null);

                                Debug.DrawRay(legstaticpivot.position, legstaticpivot.forward * someDist, Color.white, 0.1f);
