...
Shader "geoshader" 
{
	Properties 
	{
		...
		_RandomHeight("Random height", Float) = 0.01
		_RandomSpeed("Random Speed", Float) = 0.01
		_WeightValue("Weight Value", Float) = 0.0
	}
	SubShader 
	{	
    	...
    	Pass 
    	{
			...
			v2g vert(appdata_full v)
			{
				...
				//float phase0 = (_WaveHeight)* sin((_Time[1] * _WaveSpeed) + (v0.x * _WaveLength) + (v0.z * _WaveLength) + (v0.xzz));
				//float phase0_1 = (_RandomHeight)*sin(cos((v0.xzz) * _RandomHeight * cos(_Time[1] * _RandomSpeed * sin((v0.xxz)))));
				//phase0 + phase0_1
